...
{
    // Verifies the camera-relative ground basis (AxisBasis.ComputePlaneBasis) for a standard no-roll player
    // camera: stick-up moves along the camera's ground-projected forward, stick-right is screen-right with
    // Unity's left-handed convention, top-down degeneracy stays finite/orthonormal, and toggling CameraRelative
    // off does not invert controls. Ran identically as a standalone Unity.Mathematics harness during development.
    public class AxisBasisTests
    {
        private static readonly float3 Up = new float3(0, 1, 0);

        private static bool Near(float3 a, float3 b, float e = 1e-3f) => math.lengthsq(a - b) < e * e;

        [Test]
        public void CameraRelative_Yaw0_StickMapsToWorldAxes()
        {
            var cam = quaternion.Euler(math.radians(30f), 0f, 0f); // pitched down, looking +Z
        ...
        {
            var cam = quaternion.Euler(math.radians(20f), math.radians(90f), 0f); // looking +X
            AxisBasis.ComputePlaneBasis(Up, true, cam, out var forward, out var right);
        ...
        public void CameraRelative_Handedness_HoldsAcrossAllYaws()
        {
            for (var deg = 0; deg < 360; deg += 15)
            {
                ...
                AxisBasis.ComputePlaneBasis(Up, true, cam, out var forward, out var right);
                // Unity is left-handed: ground-plane right == cross(up, forward).
        ...
        {
            var cam = quaternion.Euler(math.radians(90f), 0f, 0f); // looking straight down -Y, along the plane normal
            AxisBasis.ComputePlaneBasis(Up, true, cam, out var forward, out var right);
